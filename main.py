...
async def start(event):
    buttns = [Button.url("••SuPPort••", "https://t.me/WX_PM"), Button.url("••ʀєρο••", f'{repo}')]
    py = platform.python_version()
    if event.sender.id in OP:
        await sree.send_file(
            event.chat.id,
            file="https://telegra.ph/file/025e36beb8841336e1dbc.jpg",
...
async def start(event):
    buttns = [Button.url("••ѕυρροяτ••", "https://t.me/WX"), Button.url("••ʀєρο••", f'{repo}')]
    py = platform.python_version()
    if event.sender.id in OP:
        await sree.send_file(
            event.chat.id,
...
print("JOIN @WX_PM if you facing any kind of issue!!")
